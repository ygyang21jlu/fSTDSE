...
!*qd module procedure lapack_quad_dgesv
...
!*qd subroutine lapack_quad_dgesv(a,b)
!*qd   real(qrk), intent(inout) :: a(:,:) ! In: Linear system matrix
!*qd                                      ! Out: Destroyed
!*qd   real(qrk), intent(inout) :: b(:,:) ! In: Right-half side
!*qd                                      ! Out: Solutions
!*qd   !
...
!*qd   lda  = size(a,dim=1)
!*qd   ldb  = size(b,dim=1)
!*qd   if (lda<n .or. ldb<n) stop 'lapack%lapack_quad_dgesv - crazy inputs'
!*qd   call quad_dgesv(n,nrhs,a,lda,ipiv,b,ldb,info)
!*qd   if (info/=0) then
!*qd     write (out,"('lapack%lapack_quad_dgesv failed with code ',i0)") info
!*qd     stop 'lapack%lapack_quad_dgesv failed'
!*qd   end if
!*qd end subroutine lapack_quad_dgesv
